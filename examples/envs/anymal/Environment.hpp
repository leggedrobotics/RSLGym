...
#define ENVIRONMENT_NAME anymal_example_env
...
public:
  ...
  explicit ENVIRONMENT(const std::string& resourceDir, const YAML::Node& cfg, bool visualizable) :
    RaisimGymEnvBase(resourceDir, cfg), distribution_(0.0, 0.2), visualizable_(visualizable) {
    ...
    /// Reward coefficients
    READ_YAML(double, forwardVelRewardCoeff_, cfg["forward_vel_reward_coeff"])
    READ_YAML(double, torqueRewardCoeff_, cfg["torque_reward_coeff"])
  ...
  float step(const Eigen::Ref<EigenVec>& action) final {
    ...
    forwardVelReward_ = forwardVelRewardCoeff_ * std::min(bodyLinearVel_[0], 2.0);
  ...
  void updateInfo() final {
    ...
    // info_: std::unordered_map<string, EigenVec>
  ...
  // This function will be called when info is sent from python side
  void setInfo(const std::unordered_map<std::string, EigenVec>& info) {
    for (auto &kv: info) {
      const auto& key = kv.first;
      const auto& value = kv.second;
      if (visualizable_)
        std::cout << "[setInfo] key: " << key << " value: " << value << std::endl;
